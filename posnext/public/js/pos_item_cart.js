...
posnext.PointOfSale.ItemCart = class {
  constructor({ wrapper, events, settings }) {
    ...
    this.mobile_number_based_customer =
      settings.custom_mobile_number_based_customer;
    ...
    this.custom_use_discount_percentage =
      settings.custom_use_discount_percentage;
    this.custom_use_discount_amount = settings.custom_use_discount_amount;
    this.custom_use_additional_discount_amount =
      settings.custom_use_additional_discount_amount;
    this.custom_show_incoming_rate =
      settings.custom_show_incoming_rate && settings.custom_edit_rate_and_uom;
    this.custom_show_last_customer_rate =
      settings.custom_show_last_customer_rate;
    this.custom_show_logical_rack_in_cart =
      settings.custom_show_logical_rack_in_cart &&
      settings.custom_edit_rate_and_uom;
    this.custom_show_uom_in_cart =
      settings.custom_show_uom_in_cart && settings.custom_edit_rate_and_uom;
      ...
      if (this.$branch_section.length) {
        this.$branch_section.append(`
					...
					<div class="add-branch-wrapper">
						${this.get_branch_icon()} <span class="add-branch-text">${__(
              "Add Branch",
            )}</span>
					</div>
				`);
  ...
  bind_events() {
      ...
      if (!cur_frm.doc.customer && me.mobile_number_based_customer) { // nosemgrep
        let d = new frappe.ui.Dialog({
          ...
          primary_action: function (values) {
            if (
              values["mobile_number"].length !==
              me.settings.custom_mobile_number_length
            ) {
              frappe.throw(
                "Mobile Number Length is " +
                  me.settings.custom_mobile_number_length.toString(),
              );
            }
            frappe.call({
              method:
                "posnext.posnext.page.posnext.point_of_sale.create_customer",
              args: {
        ...
        for (var xx = 0; xx < numbers.length; xx += 1) {
          numpad_num.on("click", "." + numbers[xx], function () {
            ...
            d.set_value(
              "mobile_number",
              current_value + $(this)[0].innerHTML.toString(),
            );
          });
        }
    ...
    this.$component.on("click", ".checkout-btn-held", function () {
      ...
      // prettier-ignore
      if (!cur_frm.doc.items || cur_frm.doc.items.length === 0) { // nosemgrep
        frappe.show_alert({
      ...
      if (this.custom_use_additional_discount_amount) {
        this.$add_discount_elem.html(
          `<div class="edit-discount-btn">
              ...
              discount,
            ).bold()}&nbsp;${this.events.get_frm().doc.currency} ${__(
              "discount applied",
            )}
					</div>`,
        );
  ...
  render_cart_item(item_data, $item_to_update) {
    ...
    if (me.custom_edit_rate) {
      ...
      if (me.custom_use_discount_amount) {
        this[item_data.item_code + "_discount_amount"] =
      ...
      if (this.custom_show_incoming_rate) {
        this[item_data.item_code + "_incoming_rate"] =
          frappe.ui.form.make_control({
            df: {
      ...
      if (this.custom_show_logical_rack_in_cart) {
        this[item_data.item_code + "_logical_rack"] =
          frappe.ui.form.make_control({
            df: {
      ...
      if (this.custom_show_last_customer_rate) {
        this[item_data.item_code + "_last_customer_rate"] =
          frappe.ui.form.make_control({
            df: {
      ...
      if (me.custom_use_discount_percentage) {
        this[item_data.item_code + "_discount"].set_value(
          item_data.discount_percentage,
        );
      }
      if (me.custom_use_discount_amount) {
        this[item_data.item_code + "_discount_amount"].set_value(
          item_data.discount_amount,
        );
      }
      if (me.custom_show_incoming_rate) {
        this[item_data.item_code + "_incoming_rate"].set_value(
          item_data.custom_valuation_rate,
        );
      }
      if (me.custom_show_logical_rack_in_cart) {
        this[item_data.item_code + "_logical_rack"].set_value(
          item_data.custom_logical_rack,
        );
      }
      if (me.custom_show_last_customer_rate) {
        if (me.customer_info.customer) {
          frappe
            .xcall("posnext.posnext.page.posnext.point_of_sale.get_lcr", {
      ...
      if (me.custom_show_uom_in_cart) {
        frappe
          .xcall("posnext.posnext.page.posnext.point_of_sale.get_uoms", {
            item_code: item_data.item_code,
          })
          .then((d) => {
    ...
    function get_item_barcode(item_data) {
      ...
      const barcode_placeholder_id = `barcode-${item_data.item_code.replace(
        /[^a-zA-Z0-9]/g,
        "-",
      )}`;
  ...
  render_customer_fields() {
    ...
    function handle_customer_field_change() {
      ...
      if (
        ...
        frappe.call({
          method:
            "posnext.posnext.page.posnext.point_of_sale.set_customer_info",
          args: {
