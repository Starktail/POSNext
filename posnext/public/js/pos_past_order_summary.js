...
posnext.PointOfSale.PastOrderSummary = class {
  ...
  bind_events() {
    ...
    this.$summary_container.on("click", ".send-btn", () => {
      ...
      frappe.db
        .get_value("Customer", this.doc.customer, "mobile_no")
        .then(({ message }) => {
          if (message.mobile_no) {
            ...
            const print_url = frappe.urllib.get_full_url(
              "/printview?doctype=" +
                encodeURIComponent(this.doc.doctype) +
                "&name=" +
                encodeURIComponent(this.doc.name) +
                "&format=" +
                encodeURIComponent(this.pos_profile.print_format) +
                "&no_letterhead=0" +
                "&_lang=" +
                encodeURIComponent(frappe.boot.lang) +
                "&trigger_print=1",
            );
            ...
            var field_values = this.pos_profile.custom_whatsapp_field_names.map(
              (x) => this.doc[x.field_name],
            );

            var message_body = formatString(
              this.pos_profile.custom_whatsapp_message,
              field_values,
            );
  ...
  get_condition_btn_map(after_submission) {
    ...
    return [
      ...
      {
        condition: this.doc.is_return && this.doc.docstatus === 1,
        visible_btns: ["Print Receipt", "Email Receipt", "Send Whatsapp"],
      },
    ];
  }
  ...
  async print_receipt_on_order_complete() {
    ...
    const { message } = await frappe.db.get_value("POS Profile", profile_name, [
